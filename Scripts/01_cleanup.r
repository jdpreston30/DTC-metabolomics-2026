#* 1: Cleanup
  ...
  #+ 1.7: Cleanup tumor pathology data
    #- 1.7.1: Set paths
      #! Desktop
      tumor_pathology_path <- "/Users/JoshsMacbook2015/Library/CloudStorage/OneDrive-EmoryUniversity/Research/Manuscripts and Projects/Active Projects/Thyroid Metabolomics/DTC-metabolomics-2025/Raw_Data/Final/tumor_pathology.xlsx"
      #! Laptop
      tumor_pathology_path <- "/Users/josh/Library/CloudStorage/OneDrive-EmoryUniversity/Research/Manuscripts and Projects/Active Projects/Thyroid Metabolomics/DTC-metabolomics-2025/Raw_Data/Final/tumor_pathology.xlsx"
    #- 1.7.2: Read and clean data
      tumor_pathology <- read_excel(tumor_pathology_path, sheet = "pathology") %>%
        ...
        mutate(
          T_computed = factor(
            paste0("T", gsub("^T", "", as.character(T_stage))), # ensures single T prefix
            levels = c("T1", "T2", "T3", "T4"),
            ordered = TRUE
          )
        ) %>%
        select(-c(Patient, N:Grade, T_stage)) %>%
        arrange(T_computed)
