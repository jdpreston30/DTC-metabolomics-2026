  ...
  #+ 5.1: Merge TFT with path data
    #- 5.1.1: Pull relevant path data
      merged_tumor_data_joiner <- tumor_pathology %>%
        select(Patient_ID, T_computed, LD, LVI)
    #- 5.1.2: Join with TFT_metaboanalyst_log2
      TFT_metaboanalyst_log2_path <- TFT_metaboanalyst_log2 %>%
        ...
        select(Patient_ID, T_computed, LD, LVI, Variant, everything())
  #+ 5.2: Statistical comparisons using targeted_metabolite_comparison function
    #- 5.2.1: ANOVA comparison between T_computed stages
      t_computed_results <- targeted_metabolite_comparison(
        ...
        test_type = "anova",
        exclude_cols = c("Patient_ID", "LD", "LVI", "Variant"),
        exclude_isotopes = TRUE
      )
    #- 5.2.2: ANOVA comparison between LVI groups
      lvi_results <- targeted_metabolite_comparison(
        data = TFT_metaboanalyst_log2_path,
        grouping_var = "LVI",
        test_type = "anova",
        exclude_cols = c("Patient_ID", "T_computed", "LD", "Variant"),
        exclude_isotopes = TRUE
      )
    #- 5.2.3: ANOVA comparison between Variants
      variant_results <- targeted_metabolite_comparison(
        ...
        test_type = "anova",
        exclude_cols = c("Patient_ID", "T_computed", "LD", "LVI"),
        exclude_isotopes = TRUE
      )
