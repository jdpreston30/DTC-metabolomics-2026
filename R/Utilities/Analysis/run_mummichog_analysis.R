...
#' @param p_threshold Numeric. Optional p-value threshold override (default: NULL uses top 10%)
#'
...
run_mummichog_analysis <- function(
  ...
  p_threshold = NULL        # Optional: override dynamic 10% with specific p-value
) {
  ...
  suppressWarnings({
    ...
    # Dynamic p-value threshold selection (top 10% of peaks OR user override)
    # Custom implementation of SetMummichogPvalFromPercent (fixed version)
    
    if (!is.null(p_threshold)) {
      # User provided explicit p-value threshold
      maxp <- p_threshold
      actual_cutoff <- p_threshold
      pval_method <- "User-specified p-value threshold"
      cat("✓ Using user-specified p-value threshold:", p_threshold, "\n")
    } else {
      # Use dynamic 10% method
      fraction <- 0.1  # Top 10% of peaks
      pval_method <- "Top 10% of peaks (dynamic)"
    ...
    mSet$dataSet$pval_method <- pval_method
    mSet <- SetMummichogPval(mSet, maxp)  # Fixed: pass mSet instead of NA
    cat("✓ P-value threshold method:", pval_method, "\n")
    cat("✓ P-value threshold applied:", maxp, "\n")
  ...
  pval_method_desc <- if(!is.null(mSet$dataSet$pval_method)) {
    mSet$dataSet$pval_method
  } else {
    "Top 10% of peaks (dynamic)"
  }
  
  # Create the p-value setting description
  pval_setting_desc <- if (!is.null(p_threshold)) {
    paste0("- SetMummichogPval: ", p_threshold, " (user-specified threshold)\n\n")
  } else {
  ...
  cat("Peak filtering method:", pval_method_desc, "\n")
  cat("Peak filtering threshold (applied):", peak_filter_threshold, "\n")
