...
build_table_1 <- function(data, export_path) {
  ...
  footnote_text <- "All values are displayed as mean ± SD for ratio continuous variables or n (%) for dichotomous categorical variables. Percentages for the variant columns were calculated with respect to total patients within a variant (i.e., within column), and percentages for the total column were calculated with respect to the population total.\n\nFV-PTC, follicular variant of papillary thyroid carcinoma; SD, standard deviation"
  writeData(wb, sheet = 1, x = footnote_text, startRow = merge_row_num, startCol = 1)
  
  # Add style to merged cell with footnote (italic, size 8, left/middle aligned)
  ...
  setRowHeights(wb, sheet = 1, rows = merge_row_num, heights = 55)
