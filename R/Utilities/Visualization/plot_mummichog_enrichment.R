...
plot_mummichog_enrichment <- function(
  ...
  if (combine_databases) {
    ...
    # Use generic comparison names or file names if available
    comparison_names <- names(json_files)
    if (is.null(comparison_names) || any(is.na(comparison_names))) {
      comparison_names <- paste0("Comparison_", seq_along(json_files))
    }
    pathway_data_list <- map2(json_files, comparison_names, read_mummichog_json)
  ...
  if (combine_databases) {
    ...
    all_pathways <- unique(as.character(enrichment_data$pathway_name))
    ...
    all_pathways <- unique(as.character(enrichment_data$pathway_name))
    remaining_pathways <- setdiff(all_pathways, pathway_order)
  ...
  if (n_comparisons == 1) {
    # Single column layout - use facet_grid to preserve pathway ordering
    p <- p + facet_grid(
