...
#+ 4.1 Prepare Data
  #- 4.1.1: Bring in original T-stage data
    T_stage_original <- tumor_pathology_raw %>%
      select(Patient_ID, T) %>%
      rename(T_original = T)
  #- 4.1.2: Join with T Original
    cluster_path_analysis <- variance_study %>%
      ...
      left_join(T_stage_original, by = "Patient_ID") %>%
      select(T_computed, Cluster, LVI, MFC, T_stage, Variant, T_original)
#+ 4.2: Faceted Stacked Bar Chart
  #- 4.2.1 Prepare Data for Faceted Stacked Bar Plot
    ...
    #_Test on original
      stacked_bar_orig_T <- cluster_path_analysis %>%
        count(Cluster, Variant, T_original, name = "count") %>%
        mutate(
          Cluster = factor(Cluster, levels = c("Cluster 1", "Cluster 2")),
          Variant = factor(Variant, levels = c("PTC", "FV-PTC", "FTC")),
          T_original = factor(T_original, levels = c("1", "2", "3", "4"))
        )
