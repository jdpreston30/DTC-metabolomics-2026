...
#- 1.1.3: Create Stage PCA
stage_PCA_12 <- make_PCA(
  data = UFT_filtered,
  group_var = "stage_bin",
  ellipse_colors = stage_bin_colors,
  comp_x = 1, comp_y = 2
)
#+ 1.1: Run Exploratory PLSDAs
#- 1.1.2: Create variant PLSDA
variant_pca_12 <- make_PCA(
  data = UFT_filtered,
  group_var = "Variant",
  method = "PLSDA",
...
#- 1.1.3: Create Stage PLSDA
stage_PCA_12 <- make_PCA(
  data = UFT_filtered,
  group_var = "stage_bin",
  method = "PLSDA",
  ellipse_colors = stage_bin_IvAll_colors,
  comp_x = 1, comp_y = 2
)
#+ 1.2: Create heatmaps
#- 1.2.1: Prepare data for heatmap
heatmap_data_T <- UFT_filtered %>%
  select(ID, Variant, Stage, any_of(feature_cols)) %>%
  arrange(Stage)
