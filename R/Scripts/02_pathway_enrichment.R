...
mfn_inspect <- mfn_tibbles$eva |>
  ...
  filter(p_value >= 1)
#- 2.2.4: Print MFN p-values and enrichment for visualization scaling
cat(
  "\n", strrep("=", 60), "\n",
...
mfn_enrichment_plot <- plot_mummichog_enrichment(
  ...
  p_threshold = 0.1,
  enrichment_cap = 1.75,
  size_range = c(9, 15),  # Min enrichment (1.36) maps to size 3, max (1.75) maps to size 12
  size_breaks = c(3.5, 2.5, 1.5),  # Reversed order - largest first
  show_legend = TRUE,
  save_path = "Outputs/Figures/Raw/p1C.png",
  plot_width = 5.8,
