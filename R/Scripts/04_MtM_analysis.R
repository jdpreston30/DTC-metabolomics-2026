...
cat(
  "\n", strrep("=", 60), "\n",
  "KEGG CORRELATION PLOT DATA RANGES\n",
  strrep("=", 60), "\n\n",
  "📊 Plot parameters:\n",
  "   P-value range:    ", min(correlation_kegg_plot_data$p_fisher), " to ", max(correlation_kegg_plot_data$p_fisher), "\n",
  "   Enrichment range: ", round(min(correlation_kegg_plot_data$enrichment_factor), 2), " to ", round(max(correlation_kegg_plot_data$enrichment_factor), 2), "\n",
  "   Pathways:         ", length(unique(correlation_kegg_plot_data$pathway_name)), "\n",
  "   Metabolites:      ", length(unique(correlation_kegg_plot_data$Comparisons)), "\n",
  strrep("=", 60), "\n\n"
)
#- 4.6.3: Create multi-column enrichment plot
correlation_kegg_plot <- plot_mummichog_columns(
  enrichment_data = correlation_kegg_plot_data,
  p_threshold = 0.05,
  enrichment_cap = 5,
  size_range = c(5, 10),
  color_scale = "rb",
